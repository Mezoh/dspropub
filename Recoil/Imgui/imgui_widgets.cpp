...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (GetItemID() & ImGuiItemFlags_ButtonRepeat)
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(25, 25,25, 255), 3.f);

    window->DrawList->AddRectFilled(rect.Min, rect.Max, ImColor(252, 171, 3, it_anim->second.active_rect_alpha), 3.f);
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
    ...
    ImGui::RenderFrame(frame_bb.Min, frame_bb.Max, ImGui::GetColorU32(ImVec4(0.05f, 0.05f, 0.05f, 0.54f)), true, style.FrameRounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(rect.Max - ImVec2(13, 13), rect.Max, ImColor(111, 111, 111, 54), 2.f);

    window->DrawList->AddRectFilled(rect.Max - ImVec2(13, 13), rect.Max, ImColor(252, 171, 3, it_anim->second.active_rect_alpha), 2.f);

    //RenderCheckMark(window->DrawList, rect.Max - ImVec2(12, 12), ImColor(252, 171, 3, it_anim->second.active_rect_alpha), 9.f);

    window->DrawList->AddRect(rect.Max - ImVec2(13, 13), rect.Max, ImColor(75, 75, 75, (15 + it_anim->second.active_outline_anim)), 2.f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    //window->DrawList->AddRectFilled(rect.Max - ImVec2(15, 15), rect.Max, ImColor(75, 75, 75, 54), 2.f);
    window->DrawList->AddRectFilled(clickable.Min, clickable.Max, ImColor(75, 75, 75, 30), 2.f);

    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 2.f);

    window->DrawList->AddText(rect.Min, ImColor(255, 255, 255, 200), label);

    window->DrawList->PushClipRect(clickable.Min, clickable.Max - ImVec2(29, 0));
    window->DrawList->AddText(clickable.Min + ImVec2(5, 7), ImColor(255, 255, 255, 200), preview_value);
    window->DrawList->PopClipRect();

    window->DrawList->AddCircleFilled(rect.Max - ImVec2(11, 13), 3.f + it_anim->second.active_circle_anim, ImColor(75, 75, 75, 100));
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(40, 40, 40, 255)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (!temp_input_is_active)
    {
        //const bool input_requested_by_tabbing = temp_input_allowed && (GetItemID() && ImGuiItemStatusFlags_FocusedByTabbing) != 0;
        const bool clicked = (hovered && g.IO.MouseClicked[0]);
        if (clicked || g.NavActivateId == id || g.NavActivatePressedId == id)
        {
            SetActiveID(id, window);
            SetFocusID(id, window);
            FocusWindow(window);
            g.ActiveIdUsingNavDirMask |= (1 << ImGuiDir_Left) | (1 << ImGuiDir_Right);
            if (temp_input_allowed && ((clicked && g.IO.KeyCtrl) || g.NavActivatePressedId == id))
                temp_input_is_active = true;
        }
    }
    ...
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 7.f), grab_bb.Max - ImVec2(10.f, 5.f), ImColor(75, 75, 75, 255));

    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(5.f, 6.f), 6.f + it_anim->second.circle_anim, ImColor(252, 171, 3, 255), 20.f);
